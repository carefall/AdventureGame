...
public class PlayerData : MonoBehaviour
{
    ...
    public float npcSaveDistance = 100f;
...
/*    private void Start()
    ...
    public void Fill(int hp, int level, bool male, SaveData.QuestStates[] quests, int currentQuest, int[] completedQuests, int[] failedQuests, string[] deadUniqueNPCs, InventoryItem[] items)
    {
        this.hp = hp;
        this.level = level;
        this.male = male;
...
//        deadUniqueNPCs.Add(uniqueName);
    ...
    public List<Entity> GetEntities()
    {
        ...
        Collider[] cols = Physics.OverlapSphere(transform.position, npcSaveDistance);
        foreach (Collider col in cols)
        {
            if (col.gameObject.TryGetComponent<Entity>(out Entity e))
            {
                entities.Add(e);
            }
        }
